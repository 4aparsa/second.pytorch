...
def prep_pointcloud(input_dict,
    ...
    # print('Points_1: ', np.shape(points), points)  # xumiao
    ...
    anchors_mask = None
    if anchor_area_threshold >= 0:  # 1
        coors = coordinates
        # print('0', np.shape(coors), coors)  # xumiao
        dense_voxel_map = box_np_ops.sparse_sum_for_anchors_mask(
            coors, tuple(grid_size[::-1][1:]))
        # print('1', np.shape(dense_voxel_map), dense_voxel_map)  # xumiao
        dense_voxel_map = dense_voxel_map.cumsum(0)
        # print('2', np.shape(dense_voxel_map), dense_voxel_map)  # xumiao
        dense_voxel_map = dense_voxel_map.cumsum(1)
        # print('3', np.shape(dense_voxel_map), dense_voxel_map)  # xumiao
        anchors_area = box_np_ops.fused_get_anchors_area(
            dense_voxel_map, anchors_bv, voxel_size, pc_range, grid_size)
        # print('4', np.shape(anchors_area), anchors_area)  # xumiao
...
def _read_and_prep_v10(info, root_path, num_point_features, prep_func, v):
    """read data from KITTI-format infos, then call prep function.
    """
    # velodyne_path = str(pathlib.Path(root_path) / info['velodyne_path'])
    # velodyne_path += '_reduced'
